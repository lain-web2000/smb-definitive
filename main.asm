...
VictoryModeMain:
          ...
          bne NotW8orD               ;to see which subroutine we're on
          cpx #WorldD                ;if not on world D, skip, don't bother checking
          bne NotW8orD               ;to see which subroutine we're on
          cmp #$05
...
VictoryModeSubroutines:
    ...
    cmp #World8
    beq VictoryModeSubsForW8andD  ;note that world D will also run second set of subs
    cmp #WorldD
    beq VictoryModeSubsForW8andD
...
EndCastleAward:
   ...
   beq SetWorldEndTimer   ;ANN bugfix for castle countdown, skip ahead if timer is zero
...
GetPlayerColors:
               ...
               lda #$04                 ;otherwise load offset for luigi
ChkFiery:      ldy PlayerStatus         ;check player status
               cpy #$02
               bne StartClrGet          ;if fiery, add $08 to offset for fiery player
               clc
               adc #$08
StartClrGet:   tay                      ;transfer our offset into Y
...
WarpZoneNumbers:
  ...
  .byte $24, $02, $24, $00         ; 1-2 underground main area warpzone
  .byte $24, $03, $24, $00         ; 1-2 ground subarea warpzone
  .byte $24, $04, $24, $00         ; 1-2 underground subarea warpzone
  .byte $24, $01, $24, $00         ; 3-1 warpzone
  .byte $24, $06, $24, $00         ; 5-1 warpzone
  .byte $24, $07, $24, $00         ; 5-2 underground main area warpzone
  .byte $24, $08, $24, $00         ; 5-2 ground subarea warpzone
  .byte $24, $05, $24, $00         ; 8-1 warpzone
  .byte $24, $0b, $24, $00         ; A-2 warpzone
  .byte $24, $0c, $24, $00         ; A-3 warpzone
...
SetInitNTHigh: sty CurrentNTAddr_High   ;store name table address
               ...
               bcc CheckHalfway
               bcs SetSecHard
ChkSecHard:    cmp #World4              ;if less than 4, do not activate secondary
               bcc CheckHalfway
               ...
               bcc CheckHalfway
SetSecHard:    inc SecondaryHardMode    ;set secondary hard mode flag for SMB1 5-3/SMB2J 4-4 and beyond
...
WarpsForSMB1Levels:
      ldx #$80                 ;use base number for warp to worlds 4-3-2
      lda WorldNumber          ;from happening in warp zone code elsewhere
      beq BaseW                ;if on world 1, use 4-3-2 warp
      ldy AreaType             ;otherwise check area type for ground area
      dey
      bne W5Warp2              ;if not, use world 5 warp
      beq W5Warp3              ;if found, use 8-7-6 warp

WarpWorldsAThruD:
      lda #$8a                 ;use base number for worlds A-D
      clc
      adc LevelNumber          ;add level number itself to it
      bne DumpWarpCtrl         ;then branch to use it

WarpWorlds2Thru8:
      ldx #$86                 ;use base number for worlds 2-8
      lda WorldNumber
...
FlagBalls_Residual:
      ...
      lda #$71             ;of screen downwards based on low nybble
      jmp RenderUnderPart
...
FlagpoleObject:
      ...
      lda #$25
      jsr RenderUnderPart
      lda #$64                 ;render solid block at the bottom
      sta MetatileBuffer+10
...
BulletBillCannon:
             ...
             lda #$67                 ;render bullet bill cannon
             sta MetatileBuffer,x
             inx
             dey                      ;done yet?
             bmi SetupCannon
             lda #$68                 ;if not, render middle part
             sta MetatileBuffer,x
             inx
             dey                      ;done yet?
             bmi SetupCannon
             lda #$69                 ;if not, render bottom until length expires
             jsr RenderUnderPart
...
NextStair: dec StaircaseControl      ;move onto next step (or first if starting)
           ...
           tay
           lda #$64                  ;now render solid block staircase
           jmp RenderUnderPart
...
Jumpspring:
      ...
      ldx $07
      lda #$6a                    ;draw metatiles in two rows where jumpspring is
      sta MetatileBuffer,x
      lda #$6b
      sta MetatileBuffer+1,x
NoJs: rts
...
RenderUnderPart:
             ...
             cmp #$50
             beq WaitOneRow        ;if stem top of mushroom, wait
...
JmpMove: jsr MovePlayerHorizontally ;do another sub to move player horizontally
         ...
         lda FileListNumber         ;if in SMB1 levels or 2J worlds 1-4, don't bother checking for wind
         beq ExOGSS
         jmp BlowPlayerAround
ExOGSS:  rts
...
.ifdef PAL
	...
	LuigiJumpMForceData:
		  .byte $22, $22, $22, $30, $30, $0d, $04 ;PAL diff: Faster acceleration to compensate FPS difference

	LuigiFallMForceData:
		  .byte $64, $64, $5d, $86, $86, $0a, $09 ;PAL diff: Faster acceleration to compensate FPS difference
...
.ifdef PAL
           cmp #$27                   ;PAL diff: Faster speed cutoffs to compensate FPS difference
.else
...
EraseClM:   tay
            ...
            cmp #$26                  ;if not found, move down a row
            bne NoClimbM
...
SetHSpd:  
.ifdef PAL
          lda #$fd                   ;PAL diff: Faster speed to compensate FPS difference
.else
          lda #$fe
.endif
...
ChkBrick:  bcc PutMTileB            ;if no match was found in previous sub, skip ahead
           ...
           cpy #$5e                 ;is it brick with coins (without line)?
...
BrickQBlockMetatiles:
      .byte $c1, $c2, $c0, $60, $61, $62, $63 ;used by question blocks

      .byte $54, $55, $56, $57, $58, $59 ;used by ground level bricks
      .byte $5a, $5b, $5c, $5d, $5e, $5f ;used by other level bricks
...
.ifdef PAL
             ldy #$1f                ;PAL diff: Faster speed to compensate FPS difference
.else
             ldy #$1c                ;set movement amount for podoboo/other objects
.endif
SetHiMax:    
.ifdef PAL
             lda #$04                ;PAL diff: Faster maximum speed to compensate FPS difference
.else
...
ExecGameLoopback:
      ...
      rts
ExecGameLoopbackJ:
      lda AreaDataOfsLoopbackJ,y ;adjust area object offset based on
      sta AreaDataOffset        ;which loop command we encountered
      rts
...
;--------------------------------
;$05 - used to hold world number for comparison
...
Pars0e: sta $05
        ...
        lsr
        lsr
        cmp $05                  ;is it the same world number as we're on?
...
MaxCC:   sty $00                    ;store whatever pseudorandom bits are in Y
         ...
         sta $01
.ifdef PAL
         lda #$fa                   ;PAL diff: Faster speed to compensate FPS difference
.else
...
MoveHammerBroXDir:
.ifdef PAL
         ldy #$fb                  ;PAL diff: Faster speed to compensate FPS difference
.else
...
.endif
         ...
         bne Shimmy
.ifdef PAL
         ldy #$05                  ;PAL diff: Faster speed to compensate FPS difference
.else
...
ChkNearPlayer:
      lda Enemy_Y_Position,x    ;get vertical coordinate
.ifdef PAL
      adc #$0c                  ;PAL bugfix: Bloopers can get closer vertically
.else
...
ExtD: cmp #$21                    ;otherwise, extend distance for green piranha plants
      bcc PutinPipe
...
GrnPP: lda FrameCounter            ;get frame counter
       lsr
...
ExtendLB: sbc #$48                ;subtract 72 pixels regardless of enemy object
          ...
          beq ExScrnBd
          cpy #Bowser             ;if bowser, do not erase
          beq ExScrnBd
...
BowserIdentities:
      ...
      .byte RedKoopa, GreyCheepCheep, BulletBill_CannonVar, Bowser
...
SetCollisionFlag:
      ldx ObjectOffset             ;get enemy object buffer offset
      pha                          ;ANN bugfix to prevent platform collisons from occuring
      lda Player_Y_HighPos         ;below a certain vertical position, which prevents
      cmp #$01                     ;vertical screenwrapping using moving platforms
      bne ValidPlatformCollision
      lda Player_Y_Position
      cmp #$df
      bcc ValidPlatformCollision
      pla
      rts
ValidPlatformCollision:
      pla
...
ChkPBtm:  ldy Player_State           ;get player's state
          ...
          cmp #$70                   ;otherwise check for pipe metatiles
...
ChkInvisibleMTiles:
         cmp #$60       ;check for hidden coin block
         beq ExCInvT
         cmp #$61       ;check for hidden 1-up block
         beq ExCInvT
         cmp #$62       ;check for hidden poison shroom block
         beq ExCInvT
         cmp #$63       ;check for hidden power-up block
...
ChkJumpspringMetatiles:
         cmp #$6a      ;check for top jumpspring metatile
         beq JSFnd     ;branch to set carry if found
         cmp #$6b      ;check for bottom jumpspring metatile
...
ChkForNonSolids:
       ...
       beq NSFnd
       cmp #$60       ;hidden coin block?
       beq NSFnd
       cmp #$61       ;hidden 1-up block?
       beq NSFnd
       cmp #$62       ;hidden poison shroom block?
       beq NSFnd
       cmp #$63       ;hidden power-up block?
NSFnd: rts
...
EnemyGfxHandler:
       ...
       lda WorldNumber
       cmp #$03
       bcs RPlnt                   ;use default data if in world 4+ of SMB2J
GPlnt: dey                         ;otherwise make green piranha plant
RPlnt: sty $04
...
CheckToAnimateEnemy:
      ...
      cmp #World8
      beq CheckDefeatedState   ;if so, leave the offset alone (use princess)
      cmp #WorldD              ;are we on world D?
      beq CheckDefeatedState   ;if so, leave the offset alone (use princess)
      ldx #$a2                 ;otherwise, set for mushroom retainer object instead
      lda #$03                 ;set state for mushroom retainer object
      sta $ec
...
StartTheGame:
         jsr RunLoadAreaPointer    ;update area pointer for world 1-1
         inc Hidden1UpFlag
...
LoadWindWorlds5ThruD:
      ...
      ;bcc ResetDiskIOTask
      lda FileListNumber    ;if we were already in 2J worlds 5-D, leave
      lsr
      bcs ResetDiskIOTask   ;as there's no need to load anything
      lda #$01
      sta FileListNumber    ;otherwise set filelist number
      jsr LoadGameTileset   ;load in the SMB2 tiles if we need to
...
UpsideDownPipe_Low:
       ...
       bne UDP                      ;patch for super mario bros. 1
UDPJ:  lda #$04                     ;start at fifth row
       pha
...
DemoResetOrGameOver:
       lda GameOverMsgFlag       ;if special game over message flag not set, branch
       beq GoToDemoReset
       lda OperMode              ;if not in game over mode, branch
       cmp #GameOverMode
       bne GoToDemoReset
       lda #$20
       sta ScreenTimer
...
AwardExtraLives:
    ...
    cmp #WorldD
    beq ExtraLivesPoints       ;if so, award points for extra lives
    jmp BackToNormal           ;if not, just move on to the next world
ExtraLivesPoints:
...
BlueUpdateTiming:
           ...
           bne ExFade
BlueUpd:   ldy VRAM_Buffer_Offset ;practice hack fix, respect buffer offset
           ldx #$00
BlueULoop: lda BlueTransPalette,x ;write palette to VRAM buffer
           ...
           cpx #$14
           bmi BlueULoop
           ldx VRAM_Buffer_Offset ;fix, respect buffer offset
...
EraseLivesLines:
     ldy VRAM_Buffer_Offset    ;fix, respect buffer offset
...
RunMushroomRetainers:
       ...
       bne ExRMR
       inc OperMode_Task           ;move onto the last task
ExRMR: rts
...
SChkLp: lda SaveHeader,x        ;check all bytes of the save data header
        ...
        dex
        bpl SChkLp              ;if not, loop back
        rts                     ;otherwise save data is valid, leave
InitializeSaveData:
        ; clear existing save data (TO-DO: Establish default settings)
        ldx #SaveData_End-SaveData-1
ClrSLp: lda #$00
        sta SaveData,x
        dex
		cpx #$ff
